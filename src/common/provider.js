...
export default class Provider extends EventEmitter {
  ...
  constructor(logger = console) {
    ...
    // setup own event listeners EVERIP-32
    this.on('connect', () => {
    ...
    this.on('ever_subscription', (params) => {
      this.emit('message', {
  ...
  request(args) {
    ...
    return new Promise((resolve, reject) => {
      ...
      window.dispatchEvent(new CustomEvent('pertinax-wallet-request', {
      ...
      window.addEventListener(`pertinax-wallet-response-${uniqueId}`, listener);
      ...
      let trackerRemoveEventListener = setInterval(function() {
        ...
        window.removeEventListener(`pertinax-wallet-response-${uniqueId}`, listener);
  ...
  async _initializeState() {
    try {
      ...
      // indicate that we've connected, for TIP-32 compliance
      ...
      this._log.error(
        'Pertinax wallet: Failed to get initial state. Please report this bug.',
        error,
      );
    } finally {
  ...
  _handleEndpointChanged(endpoint) {
    if ((!endpoint && endpoint != null) || endpoint == "") {
      this._log.error(
        'Pertinax Wallet: Received invalid network parameters. Please report this bug.',
        endpoint,
      );
      return;
    }
  ...
  _handleUnlockStateChanged({ account, isLocked }) {
    if (typeof isLocked !== 'boolean') {
      this._log.error(
        'Pertinax Wallet: Received invalid isLocked parameter. Please report this bug.',
      );
      return;
    }
